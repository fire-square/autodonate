...
Мы используем `pytest` и `flake8` для контроля качества.
Мы так же используем [wemake_python_styleguide](https://github.com/wemake-services/wemake-python-styleguide) для обеспечения качества кода.
...
flake8 .
```
Держите в виду: Папка виртуальной среды по умолчанию, исключенная линтером flake8, это `.venv`.
Если вы хотите настроить этот параметр, вы должны сделать это в `setup.cfg`.
